...
export const OrdersTable: React.FC = () => {
  ...
  const [filter, setFilter] = useState<'all' | 'Solicitado' | 'Contestado'>('all');
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errJson?.error || `Error ${res.status}: no se pudieron obtener los pedidos.`);
      }
  ...
  const handleStatusToggle = async (orderId: string, newStatus: OrderStatus) => {
    if (!token) return;
    try {
  ...
  // Metrics calculation
  const totalSolicitados = orders.filter((o) => o.status === 'Solicitado').length;
  const totalContestados = orders.filter((o) => o.status === 'Contestado').length;
  const totalVentas = orders.reduce((sum, o) => sum + (o.totalAmount || 0), 0);
  const totalBulks = orders.reduce(
    (sum, o) => sum + o.items.filter((i) => i.purchaseType === 'bulk').reduce((acc, i) => acc + i.quantity, 0),
    0
  );
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white p-4 rounded-2xl border border-gray-200 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <div className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#f7f7f7] border border-gray-200 rounded-xl text-xs font-semibold text-gray-700">
            ...
            <span>
              Hoy: {new Date().toLocaleDateString('es-AR', { day: '2-digit', month: 'short', year: 'numeric' })}
            </span>
          </div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl shadow-xs overflow-hidden">
            ...
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredOrders.map((order) => {
                  ...
                  const bulks = order.items.filter((i) => i.purchaseType === 'bulk').reduce((a, i) => a + i.quantity, 0);
                  const units = order.items.filter((i) => i.purchaseType === 'unit').reduce((a, i) => a + i.quantity, 0);
                  const initials = `${order.customer.firstName.charAt(0)}${order.customer.lastName.charAt(0)}`.toUpperCase();
                  ...
                  return (
                    <tr key={order._id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="py-3 px-4 whitespace-nowrap">
                        <button
                          onClick={() =>
                            handleStatusToggle(order._id, isSolicitado ? 'Contestado' : 'Solicitado')
                          }
                        ...
                        >
                          <span
                            className={`w-2 h-2 rounded-full ${
                              isSolicitado ? 'bg-[#e65100] animate-pulse' : 'bg-[#2e7d32]'
                            }`}
                          />
        ...
        <div className="p-3 bg-[#fbf9f8] border-t border-gray-200 text-xs text-gray-500 flex items-center justify-between">
          <span>
            Mostrando {filteredOrders.length} de {orders.length} pedidos mayoristas
          </span>
          <span className="font-semibold text-gray-700">Canal Directo de Despacho DistriQuiero</span>
        </div>
      </div>
